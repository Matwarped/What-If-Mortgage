...
<div class="max-w-5xl mx-auto p-6">
    <div class="bg-white rounded-2xl shadow p-6">
        <h1 class="text-2xl font-semibold mb-2">What If Mortgage</h1>
        ...
        <div class="mb-6">
            <h3 class="font-semibold mb-2">Balance Over Time</h3>
        ...
        <div class="mb-6">
            <h3 class="font-semibold mb-2">Loan Distribution</h3>
        ...
        <div class="mb-6">
            <h3 class="font-semibold mb-2 text-lg">Comparison Table</h3>
            <table class="w-full text-sm border border-slate-200 rounded">
                <thead class="bg-slate-100">
                    <tr>
                        ...
                        <th class="p-2 text-left">Payoff Time</th>
                        <th class="p-2 text-left">Total Interest</th>
                        <th class="p-2 text-left">Total Repayment</th>
                        <th class="p-2 text-left">Interest Saved</th>
                        <th class="p-2 text-left">Months Saved</th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="border-t">
                        <td class="p-2">Baseline</td>
                        <td class="p-2 text-left">
                            {monthlyPayment.toLocaleString()}
                        </td>
                        <td class="p-2 text-left">{fmtYearsMonths(baseline.months)}</td>
                        <td class="p-2 text-left">{fmtMoney(baseline.totalInterest)}</td>
                        <td class="p-2 text-left">{fmtMoney(balance + baseline.totalInterest)}</td>
                    ...
                    {#snippet compareRow(name, value, compareValue, current, previous)}
                        <tr class="border-t">
                            ...
                            <td class="p-2 text-left">{fmtMoney(baseline.totalInterest - current.totalInterest)}</td>
                            <td class="p-2 text-left">{fmtMoney(baseline.months - current.months)}</td>
                        </tr>
                    {/snippet}
    ...
    <footer class="text-xs text-slate-500 mt-4">
        Built for quick planning — numbers are approximate and ignore past
        progressive releases or moratorium distortions.
    </footer>
</div>
